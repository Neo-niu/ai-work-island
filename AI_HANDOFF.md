...
- 2026-08-12：0.5.16 build 48 新增 Codex 运行进度展开界面，从 rollout 增量读取 commentary 与 `update_plan`，展示当前/最近活动及明确步骤计数；新任务开始时重置旧进度，不虚构百分比。兼容旧 `function_call` 与当前桌面版 `custom_tool_call/exec` 两种计划事件。118 项测试、构建安装、严格签名、进程、rollout/自动化诊断和安装版辅助功能树回读通过；真实卡片回读到“正在”及两条“刚刚”动态。
