...
- 每张 Codex 任务卡悬停后提供“转到 Codex”。工作岛自建任务仍在执行时，按钮先关闭该 thread 对应的后台 App Server、释放工作岛占用，再等待同标题任务真实进入 Codex 可见侧栏后按目标行打开；不再回退会广播到隐藏窗口的 `codex://` 深链。侧栏 8 秒内仍未同步时保留待读状态并提示稍后重试，不误报“会话不存在”，也不提前标记已阅。后台进程按 thread ID 登记，任务自然结束和主动转移都会清理登记，避免误杀其他任务。
