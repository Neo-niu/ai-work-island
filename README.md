...
- **悬浮胶囊**：以公司额度为默认主额度，Codex 额度仅在本机存在有效账号数据时追加显示；异常、失联或等待确认时锁定重要状态。
...
每位使用者需在自己的 Mac 上单独完成一次配置：使用 Microsoft Edge 登录公司模型平台 `https://model.zhenguanyu.com/`，并保留一个已登录的平台标签页。AI 工作岛只通过该标签页的同源请求读取当前用户额度，不读取或保存 Cookie。未配置时界面会显示“公司额度待配置”；没有 Codex 账号不会影响公司额度展示。
...
- 公司额度读取依赖 Edge 中已登录的平台页面；未配置时显示明确引导。
- Codex 额度为可选信息，仅在本机能读到有效额度数据时显示。
